...
public interface BlockRepository extends PagingAndSortingRepository<Block, Integer> {
	...
	@Query(value = "select * from tmblock where blkName = ?1", nativeQuery = true)
	Block findByBlockName(String blkName);
	Block findBlockByblkId(Integer blkId);
	...
	int  findByRefBranchIdAndBlkName(Integer refBranchId,String blkName);

	@Query(value = "SELECT tb.blkId,tb.blkName,tb.blkGPSRadius,tb.blkLatLong,tmc.custId FROM  tmblock tb LEFT JOIN tmbranch  tbr ON tbr.brId = tb.refBranchId LEFT JOIN  tmcustomer tmc ON tmc.custId = tbr.refCustomerId WHERE  tmc.custId=?1 AND tb.refBranchId=?2" ,nativeQuery = true) 
